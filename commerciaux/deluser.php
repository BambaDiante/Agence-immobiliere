...
<body>
    <?php
        if(isset($_POST['idUser']) || isset($_POST['confirm'])){
            $id=$_POST['idUser'] ?? $_POST['hidden_id'] ;
            echo "<form method='POST' id='valider' action=''>";
            echo "<input type='hidden' name='hidden_id' value='".$id."'>";
            echo "<input type='submit'  name='confirm' value='Valider la desactivation'>";
            echo "</form>";   
            if(isset($_POST['confirm'])){
                $des="UPDATE users SET is_activated = 0 WHERE IdUser = :IdUser";
                ...
                if($result){
                    echo "<div class='bloc'>";
                    echo "<p>Le compte a ete desactive avec succes</p>";
                    echo "</div>";
                    header("Location: gestionclient.php");
                }
                else{
                    echo "<p>Erreur lors de la desactivation</p>";
                }
            }
        }
